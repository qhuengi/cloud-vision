...
def brMap(image):
    ...
    mapped = np.array([list(map(pixelMap, col)) for col in image],dtype='float32')
    return mapped
...
def split(img, rect, sdthreshold = 0.13,sizethreshold = 5000,brThresh = 0.3):
    '''Recursively splits bounding rectangle into homogenous rects, and filters non-cloud rects.
    ...
    brThresh -- max average intensity of pixels in rectangle, i.e. if avg intensity is greater, rect will be filtered (default 0.3).
    '''
    ...
    flattened = flatten(rects,list)
    return list(filter(filterRect, flattened))
